...
class ZooRouter implements RouterInterface {
	...
	$task = 'alphaindex';
		...
		if ((@$query['task'] == $task || @$query['view'] == $task) && isset($query['alpha_char'], $query['app_id'])) {
            ...
            if (!($menu = $app->system->application->getMenu('site')
                    and $menu instanceof AbstractMenu
...
function ZooParseRoute(&$segments) {
	...
	// fix segments (see Router::_decodeSegments)
	...
	$task = 'alphaindex';
        ...
        if ($count == 2 && $segments[0] == $task && $menuItem = $app->menu->getActive()) {
            $vars['task']       = $task;
            $vars['alpha_char'] = (string) $segments[1];
            $vars['app_id'] = $menuItem->getParams()->get('application');
        }

        if ($count == 3 && $segments[0] == $task) {
            $vars['task']           = $task;
            if (!$vars['app_id'] = (int) $app->alias->application->translateAliasToID($segments[1]) and $menuItem = $app->menu->getActive()) {
                $vars['app_id']     = $menuItem->getParams()->get('application');
                $vars['alpha_char'] = (string) $segments[1];
                $vars['page']       = (int) $segments[2];
            } else {
	...
	if (empty($vars)) {
		if (($count == 0 || $count == 1 && is_numeric($segments[0])) && $menuItem = $app->menu->getActive()) {
			$vars['task'] = $vars['view'] = @$menuItem->query['view'];
    ...
    array_splice($segments, 0, count($vars));
