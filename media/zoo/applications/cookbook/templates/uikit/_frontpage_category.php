	...
	<?php
		...
		if ($this->params->get('template.show_sub_categories') && $selected_child_categories = $category->getChildren()) {
			...
			if (!$this->params->get('config.show_empty_categories', false)) {
				$selected_child_categories = array_filter($selected_child_categories, function($category) { return $category->totalItemCount(); });
			}
			...
			if ($count) {
				...
				for ($j = 0; $j < $count; $j++) {
					echo '<div class="uk-width-small-1-2 uk-width-medium-1-'.$count.'"><ul class="uk-list">'.$columns[$j].'</ul></div>';
				}
				echo '</div>';
			}
