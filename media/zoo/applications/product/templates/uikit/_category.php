...
<?php if ($category) : ?>
	...
	<div class="uk-margin">
		<a href="<?php echo $link; ?>" title="<?php echo $category->name; ?>">
			<img class="<?php echo 'uk-align-'.($this->params->get('template.alignment') == "left" || $this->params->get('template.alignment') == "right" ? 'medium-' : '').$this->params->get('template.alignment'); ?> uk-margin-bottom-remove" src="<?php echo $image['src']; ?>" title="<?php echo $category->name; ?>" alt="<?php echo $category->name; ?>" <?php echo $image['width_height']; ?>/>
		</a>
	</div>
	<?php endif; ?>
