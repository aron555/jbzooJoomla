...
class ElementTextarea extends ElementRepeatable implements iSubmittable {
	...
	*/
	protected function _getSearchData() {
		...
		// clean html tags
		$value  = InputFilter::getInstance()->clean($this->get('value', ''));
	...
	*/
	public function render($params = array()) {
        $params = $this->app->data->create($params);
		return $this->app->element->applySeparators($params->get('separated_by'), $this->getValue($params));
	}
	...
	*/
	...
	protected function _addEditor($index, $value = '', $trusted_mode = true) {
		...
		if ($trusted_mode) {
			$html[] = $this->app->editor->display($this->getControlName('value', false, $index), htmlspecialchars($value, ENT_QUOTES, 'UTF-8'), '', '', self::COLS, self::ROWS, array('pagebreak', 'readmore', 'article'), "elements{$this->identifier}{$index}value");
        } else {
			$html[] = $this->app->html->_('control.textarea', $this->getControlName('value', false, $index), $value, 'class="form-control" cols='.self::COLS.' rows='.self::ROWS);
		}
		$html[] = '</div>';
		return implode("\n", $html);
	}
