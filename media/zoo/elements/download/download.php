...
class ElementDownload extends ElementFile implements iSubmittable {
	...
	*/
	public function __construct() {
		...
		if ($this->app->system->application->isClient('administrator')) {
	...
	*/
	public function render($params = array()) {
		...
		if ($layout = $this->getLayout()) {
			return $this->renderLayout($layout, $value + array(
                'download_name' => $this->app->string->str_ireplace('{filename}', $value['filename'], $params->get('download_name', '')),
	...
	*/
	public function download($check = '') {
		...
		if (!$this->canAccess()) {
			...
			echo Text::_('Unable to access download').'!';
			return;
		}
	...
	*/
	public function edit(){
		...
		// create info
		$info[] = Text::_('Size').': '.$this->getSize();
		$info[] = Text::_('Hits').': '.(int) $this->get('hits', 0);
		$info   = ' ('.implode(', ', $info).')';
	...
	*/
	public function renderSubmission($params = array()) {
        ...
        $lists = array();
        if ($trusted_mode) {
            $options = array($this->app->html->_('select.option', '', '- '.Text::_('Select File').' -'));
            if (!empty($upload) && !$this->_inUploadPath($upload)) {
                $options[] = $this->app->html->_('select.option', $upload, '- '.Text::_('No Change').' -');
            }
