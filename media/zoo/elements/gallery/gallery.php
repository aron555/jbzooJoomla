...
class ElementGallery extends Element implements iSubmittable{
	...
	protected $filter = "/\.(bmp|gif|jpe?g|png|webp|avif)$/i";
    ...
    */
    public function getValue($params = array()) {
        ...
        return $this->_getThumbnails($params);
    }
	...
	*/
	public function render($params = array()) {
		...
		// no thumbnails found
		if (empty($thumbs)) {
			return Text::_('No thumbnails found');
		}
	...
	protected function _getThumbnails($params) {
		...
		foreach ($this->getFiles() as $filename) {
			...
			if (is_file($thumb)) {
				...
				$name = !empty($title) ? $title : $this->app->string->ucwords($this->app->string->str_ireplace('_', ' ', File::stripExt($filename)));
				...
				$thumbs[] = array(
					...
					'filename'     => $filename,
					'img'          => Uri::root().$this->app->path->relative($file),
					'img_file'     => $file,
					'thumb'		   => Uri::root().$this->app->path->relative($thumb),
					'thumb_file'   => $thumb,
