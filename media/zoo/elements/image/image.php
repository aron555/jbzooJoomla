...
class ElementImage extends Element implements iSubmittable {
	...
	*/
	...
	{
		return $file = $this->get('file') and File::exists(JPATH_ROOT."/{$file}");
	}
	...
	*/
	public function render($params = array()) {
		...
		$link   = Uri::root() . $this->app->path->relative($file);
	...
	*/
	public function bindData($data = array()) {
		...
		$file = $this->get('file');
		if ($file && is_string($file) && $filepath = $this->app->path->path('root:'.$file)) {
