...
class ParameterFormHelper extends AppHelper {
	 ...
	 *
	 * @param AppParameterForm|array $params
...
class AppParameterForm {
	...
	public function render($control_name = 'params', $group = '_default') {
		...
		if ($description = (string) $this->_xml[$group]->attributes()->description) {
			$html[] = '<li class="description">'.Text::_($description).'</li>';
		}
			...
			if ($type != 'hidden') {
				...
				if ((string) $param->attributes()->label != '') {
					...
					if ((string) $param->attributes()->description != '') {
						...
						$attributes['title'] = Text::_($param->attributes()->label).'::'.Text::_($param->attributes()->description);
					}
					$output = sprintf('<label %s>%s</label>', $this->app->field->attributes($attributes), Text::_($param->attributes()->label));
				}
