...
class ZooHelper extends AppHelper {
	 ...
	 * @since 2.0
	 */
	public function toolbarHelp($ref = 'https://yootheme.com/zoo/documentation') {
		ToolBar::getInstance('toolbar')->appendButton('Link', 'help', 'Help', $ref);
	}
	...
	public function resizeImage($file, $width, $height) {
		...
		if (!Folder::exists(dirname($thumbfile))) {
			Folder::create(dirname($thumbfile));
		}
	...
	public function triggerContentPlugins($text, $params = array(), $context = 'com_zoo') {
		...
		if ($this->app->document->getType() == 'feed' && $this->app->parameter->create(PluginHelper::getPlugin('content', 'loadmodule')->params)->get('enabled', 1)) {
			$text = preg_replace('/{loadposition\s*.*?}/i', '', $text);
		}

		return $this->app->html->_('content.prepare', $text, $params, $context);
	}
	...
	public function buildPageTitle($title) {
		...
		if ($dir == 1) {
			return Text::sprintf('JPAGETITLE', $this->app->system->config->get('sitename'), $title);
		} else if ($dir == 2) {
			return Text::sprintf('JPAGETITLE', $title, $this->app->system->config->get('sitename'));
		}
		return $title;
	}
	...
	public function putIndexFile($dir) {
		...
		if (!File::exists($dir.'/index.html')) {
			...
			File::write($dir.'/index.html', $buffer);
		}
	}
