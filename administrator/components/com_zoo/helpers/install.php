...
class InstallHelper extends AppHelper {
	...
	public function uninstallApplication(Application $application) {
		...
		if (!($directory = $this->app->path->path("applications:$group")) || !Folder::delete($directory)) {
	...
	public function installApplicationFromUserfile($userfile) {
		...
		if (!File::upload($userfile['tmp_name'], $archivename, false, true)) {
        ...
        if (!$archive->extract($archivename, $extractdir)) {
	...
	public function installApplicationFromFolder($folder) {
		...
		if (!Folder::copy($folder, $write_directory, '', true)) {
