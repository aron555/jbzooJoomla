...
class HTMLHelper extends AppHelper {
	...
	public function _($type) {
        ...
        if (version_compare(JVERSION, '4.0', '>')) {
            ...
            if ($type === 'behavior.tooltip') {
                $type = 'bootstrap.tooltip';
            }

            if ($type === 'behavior.modal') {
                $type = 'bootstrap.modal';
            }

            if ($type === 'behavior.framework') {
		...
		return call_user_func_array([Joomla\CMS\HTML\HTMLHelper::class, '_'], $args);
	}

    public function __call($method, $args) {
        return call_user_func_array([Joomla\CMS\HTML\HTMLHelper::class, $method], $args);
    }
	...
	public function calendar($value, $name, $id, $attribs = null, $time = false) {
		...
		if (!defined('ZOO_CALENDAR_SCRIPT_DECLARATION')) {
			...
			foreach ($translations as $key => $translation) {
				$translations[$key] = is_array($translation) ? array_map(array('JText', '_'), $translation) : Text::_($translation);
			}
		...
		return '<div><input'.$data.' type="text" name="'.$name.'" id="'.$id.'" value="'.htmlspecialchars($value, ENT_COMPAT, 'UTF-8').'" '.$attribs.' />'
				.'<img src="'. $this->app->path->url('assets:images/calendar.png') .'" class="zoo-calendar" style="margin-left: -20px" /></div>';
	}
	...
	public function countrySelectList($countries, $name, $selected, $multiselect) {
		...
		$options = array();
		if (!$multiselect) {
			$options[] = $this->app->html->_('select.option', '', '-'.Text::_('Select Country').'-');
		}

		foreach ($countries as $key => $country) {
			$options[] = $this->app->html->_('select.option', $key, Text::_($country));
		}
	...
	public function typeList($application, $options, $name, $attribs = null, $key = 'value', $text = 'text', $selected = null, $idtag = false, $translate = false, $filter = array()) {
		...
		foreach ($application->getTypes() as $type) {
			if (empty($filter) || in_array($type->id, $filter)) {
				$options[] = $this->_('select.option', $type->id, Text::_($type->name));
			}
		}
	...
	public function moduleList($options, $name, $attribs = null, $key = 'value', $text = 'text', $selected = null, $idtag = false, $translate = false, $published = false) {
		...
		return Text::_("There are no modules to choose from.");
	}
	...
	public function pluginList($options, $name, $attribs = null, $key = 'value', $text = 'text', $selected = null, $idtag = false, $translate = false, $folder = false, $enabled = true) {
		...
		if (count($plugins)) {
			...
			foreach ($plugins as $plugin) {
				if ($enabled && PluginHelper::isEnabled($plugin->type, $plugin->name)) {
					$plugin_name = 'plg_'.$plugin->type.'_'.$plugin->name;
					Factory::getApplication()->getLanguage()->load($plugin_name.'.sys', JPATH_ADMINISTRATOR);
		...
		return Text::_("There are no plugins to choose from.");
	}
	...
	public function accessLevel($options, $name, $attribs = 'class="inputbox"', $key = 'value', $text = 'text', $selected = null, $idtag = false, $translate = false, $exclude = array()) {
		...
		foreach ($groups as $group) {
			$options[] = $this->_('select.option', $group->id, Text::_($group->name), $key, $text);
		}
	...
	public function genericList($data, $name, $attribs = null, $optKey = 'value', $optText = 'text', $selected = null, $idtag = false, $translate = false) {
		...
		return '<div>'.$this->app->html->_('select.genericlist', $data, $name, $attributes).'</div>';
	}
	...
	/**
	 * Returns select option as HTMLHelper compatible array.
	 *
	 * @param array $array
	 * @param string $value
	 * @param string $text
	...
	public function selectdirectory($directory, $filter, $name, $value = null, $attribs = null) {
		...
		// get directories
		$options = array($this->app->html->_('select.option', '', '- '.Text::_('Select Directory').' -'));
	 ...
	 * @since 2.0
	 */
	public function text($name, $value = '', $attribs = null) {
		$value = htmlspecialchars(isset($value) ? $value : '', ENT_QUOTES, 'UTF-8');
	...
	{
		$arr = array($this->app->html->_('select.option', '0', Text::_($no)), $this->app->html->_('select.option', '1', Text::_($yes)));
