	...
	*/
	public function save($object) {
		...
		if ($object->alias == '' || $object->alias != $this->app->string->sluggify($object->alias)) {
			throw new CategoryTableException('Invalid alias');
		}

		if ($this->app->alias->category->checkAliasExists($object->alias, $object->id)) {
			throw new CategoryTableException('Alias already exists, please choose a unique alias');
		}
        ...
        // trigger save event
        $this->app->event->dispatcher->notify($this->app->event->create($object, 'category:save', compact('new')));
