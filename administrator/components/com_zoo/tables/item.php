...
class ItemTable extends AppTable {
	...
	*/
	public function save($object) {
		...
		if ($object->alias == '' || $object->alias != $this->app->string->sluggify($object->alias)) {
			throw new ItemTableException('Invalid alias');
		}
	...
	*/
	...
	/*
		Function: getByIds
			Method to retrieve items by id.
	...
	*/
	public function getByIds($ids, $published = false, $user = null, $orderby = '', $ignore_order_priority = false){
		...
		// get item ordering
		list($join, $order) = $orderby == 'ids'
            ? array(false, 'FIELD (a.id,'. implode(",", $ids) .')')
            : $this->_getItemOrder($orderby, $ignore_order_priority);
	...
	*/
	...
	protected function _getItemOrder($order, $ignore_order_priority = false) {
        ...
        $orderByElement = array_filter($order, function($a) { return strpos($a, "_item") === false; });

        if ($orderByElement) {
            $result[0] = " LEFT JOIN ".ZOO_TABLE_SEARCH." AS s ON a.id = s.item_id AND s.element_id IN ('".implode("', '", $orderByElement)."')";
        }

        $order = array_diff($order, $orderByElement);
        $order = str_replace('_item', '', array_pop($order) ?: '') ?: 'name';

        $order = $orderByElement
            ? "CASE WHEN ISNULL(s.value) THEN a.{$order} ELSE s.value END"
            : "a.$order";

        if ($alphanumeric) {
            $result[1] = $orderDir == 'ASC'
                ? "{$order}+0<>0 DESC, {$order}+0, {$order}"
                : "{$order}+0<>0, {$order}+0 DESC, {$order} DESC";
        } else {
            $result[1] = "{$order} {$orderDir}";
        }
