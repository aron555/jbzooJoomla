...
class AppValidatorFile extends AppValidator {
    ...
    public function clean($value) {
        ...
        $value['name'] = File::makeSafe($value['name']);
        ...
        if ($this->hasOption('max_size') && $this->getOption('max_size') < (int) $value['size']) {
			throw new AppValidatorException(sprintf(Text::_($this->getMessage('max_size')), ($this->getOption('max_size') / 1024)));
        }
	...
	protected function _getGroupMimeTypes($group) {
		...
		$mime_types = array_filter($mime_types, function ($a) use ($group) {
            return preg_match("#^{$group}/#i", $a);
        });
